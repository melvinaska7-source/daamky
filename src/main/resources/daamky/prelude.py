...
from pydaamky.utility.render import ColorRGBA, PyAssets, PyRender3D, PyShaders
from pydaamky.ui import PyScreen
...
if not getattr(sys, "__daamky_sandbox__", False):
    _sandbox_install()
    sys.__daamky_sandbox__ = True
...
def __daamky_make_ns(_events, _client, _mc, _gv, _py_aura=None, _py_rotations=None):
    if _py_aura is None:
        from pydaamky.classes.aura import PyAura
        _py_aura = PyAura()
    if _py_rotations is None:
        from pydaamky.classes import PyRotations
