# Daamky Script API — полный справочник

Источник правды по коду клиента: `prelude.py` (формирует неймспейс), Java-классы `pydaamky/**`, загрузчик `systems/python/**`, команда `.py`.
...
- **Классы вне маппингов** (наши события, `moscow.daamky.*`, java.*, brigadier, authlib, joml) мост зовёт по имени **как есть** — это рабочий путь, но имя должно совпадать буква в букву. Для имени сущности всё равно надёжнее `getNameForScoreboard()`, чем `getName().getString()`.
...
`isinstance(...)` и `obj.getClass().getName()` в релизе молча не сработают. `is_instance` ловит и наследников (`$Full`, `$LookAndOnGround`, ...). Старый вызов `McBridge.isInstance(obj, fqn)` (`from pydaamky import McBridge`) тоже работает.
...
assets.resource("gui/icon.png")         # ТОЛЬКО ресурсы клиента (assets/daamky/...)
...
paths.root()        # <gameDir> = .minecraft/Daamky (папка клиента)
...
> `assets.resource(path)` достаёт **только ресурсы клиента** (`assets/daamky/…`). Ванильные текстуры Minecraft через него не получить — для них нужен `Identifier` (`jimport("net.minecraft.util.Identifier")`).
...
client.gameDir()          # папка КЛИЕНТА: .minecraft/Daamky (== paths.root()), а НЕ папка игры
...
profile.client; profile.version   # "Daamky", "2.1"
...
Цели: `players` (виды `local`, `friends`, `daamky_users`, `others`), `mobs`, `animals`, `items`.
...
> `ctx.target` может быть `None` — проверяй. `ctx.handler` и `aura.module` — обёртки над классами КЛИЕНТА (`moscow.daamky.*`): мост зовёт их методы по именам как есть.
