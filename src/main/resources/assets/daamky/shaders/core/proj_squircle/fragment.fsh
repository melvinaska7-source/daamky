...
void main() {
    ...
    float distance = roundedBoxSDF(center - gl_FragCoord.xy, center - 1.0, Radius, CornerSmoothness);
    float alpha = 1.0 - smoothstep(1.0 - sdfAA(distance, Smoothness), 1.0, distance);
